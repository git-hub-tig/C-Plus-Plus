 ...
 * We can perform basic CRUD operations as well as other operations like
 * sorting etc.
 *
 * ### Algorithm
 ...
 * You can insert element, sort them in order, search efficiently, delete
...
namespace data_structures
{
    ...
    namespace list_array
    {
         ...
         */
        template <uint64_t N> struct list
        {
             ...
             */
            uint64_t
            BinarySearch (const std::array<uint64_t, N> &dataArr,
                          const uint64_t &first, const uint64_t &last,
                          const uint64_t &val)
            {
                std::cout << std::endl<<__func__ << ":" << __LINE__
                          << ": Undefined condition. "<< __cplusplus << " " << __TIME__<<"\n";
                // If both pointer cross each other means no element present in
                // the list which is equal to the val
                if (last < first)
                {
                    return -1;
                }
                uint64_t mid = (first + last) / 2;
                // check whether current mid pointer value is equal to element
                // or not
                if (dataArr[mid] == val)
                    return mid;
                // if current mid value is greater than  element we have to
             ...
             */
            uint64_t
            LinearSearch (const std::array<uint64_t, N> &dataArr,
            ...
            sort ()
            {
                ...
                for (uint64_t i = 0; i < top; i++)
                {
                    ...
                    {
                        // check whether any element less than current min
                        // value
            ...
            insert (const uint64_t &val)
            {
                ...
                else
                {
                    ...
                    data[pos] = val; // Assign the value to the correct index
                                     // in the array
                }
            }
            ...
            remove (const uint64_t &val)
            {
                ...
                std::cout << "\n"
                          << data[pos]
                          << " deleted"; // print the appropriate message
