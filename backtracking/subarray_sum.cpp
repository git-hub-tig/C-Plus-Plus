 ...
 * using deletion operation at the end or the front of an array only. The
 * parent array is also counted in subarrays having 0 number of deletion
 * operations.
 *
...
namespace backtracking
{
    ...
    namespace subarray_sum
    {
         ...
         */
        uint64_t
        subarray_sum (int64_t sum, const std::vector<int64_t> &in_arr)
        {
...
test ()
{
    ...
    assert (backtracking::subarray_sum::subarray_sum (0, array1)
            == 1); // first argument in subarray_sum function is the required
                   // sum and second is the input array, answer is the subarray
                   // {(-3,-2,5)}
