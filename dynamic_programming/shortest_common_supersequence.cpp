/**
 * @file
 * @brief SCS is a string Z which is the shortest supersequence of strings X
 * and Y (may not be continuous in Z, but order is maintained).
 *
 * @details
 * The idea is to use lookup table method as used in LCS.
 * For example: example 1:-
 * X: 'ABCXYZ', Y: 'ABZ' then Z will be 'ABCXYZ' (y is not continuous but in
 * order)
 *
 ...
 * @see more on
 * [SCS](https://en.wikipedia.org/wiki/Shortest_common_supersequence_problem)
 * @see related problem
...
namespace dynamic_programming
{
    ...
    namespace shortest_common_supersequence
    {
        ...
        /**
         * Function implementing Shortest Common Super-Sequence algorithm using
         ...
         */
        std::string
        scs (std::string &str1, const std::string &str2)
        // scs (const std::string &str1, const std::string &str2)
        {
            ...
            std::vector<std::vector<int> > lookup (
                str1.length () + 1, std::vector<int> (str2.length () + 1, 0));
            ...
            while (i > 0 && j > 0)
            {
                ...
                // otherwise we check lookup table for recurrences of
                // characters
                else
                {
            ...
            // reversing the string before returning gives us the correct
            // output
...
test ()
{
    ...
    std::vector<std::string> expectedOutput{ "ABCXYZ", "ABCXYZ", "AGGXTXAYB",
                                             "XY" };
